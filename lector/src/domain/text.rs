...
fn fetch_chapter(text_handler: &mut TextHandler){
    ...
    spawn(async move{
        ...
        match html{
            Ok(txt)=>{
                ...
                render_next_page(&mut text_handler);
            },
...
pub fn find_sentence_offset(chapter_html: &str, start_snippet: &str) -> usize {
    tracing::debug!("chapter: {}",chapter_html);
    ...
    tracing::debug!("first: {}",first_sent);
    ...
    while candidates.len() > 1 {
        candidates.retain(|&(start, len)| {
            let mut cur_end = start + len;
            for i in 1..snippet_sents.len() {
                let slice_start=cur_end-10;
                let slice_len = snippet_sents[i].len() + 50;
                let end_slice = (cur_end + slice_len).min(chapter_html.len());
                let html_slice = strip_html(&chapter_html[slice_start..end_slice]);

                if normalize_text(&html_slice).contains(&normalize_text(&snippet_sents[i])) {
                    cur_end =end_slice;
                } else {
                    tracing::debug!("error on: {}",normalize_text(&html_slice));
                    tracing::debug!("trying to find:{}",normalize_text(&snippet_sents[i]) );
                    return false;
                }
            }
            true
...
pub fn trim_overflowing_node(text_handler: &mut TextHandler){
    ...
    tracing::debug!("outside? {}", child.1);

    if child.1{
        text_handler.start_text.set(strip_html(&child.0.inner_text()));
        ...
        text_handler.start_text.set(hid.clone());
        tracing::debug!("hid: {}",hid);
    }
}
...
pub fn split_and_hide_node_in_chapter(
    ...
    tracing::debug!("outer: {}",new_outer_html);
