...
pub fn last_fitting_char(layout: &LayoutQuery, page_bottom: f64) -> FitResult {
    ...
    for i in 0..layout.text_len() {
        let char_bottom = (layout.get_char_bottom)(0,i);
        if char_bottom <= page_bottom {
            tracing::debug!("text: {}",layout.text.chars().take(i as usize).collect::<String>());
            tracing::debug!("i: {}, Char bottom: {}", i, char_bottom);
