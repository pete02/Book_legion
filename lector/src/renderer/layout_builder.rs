...
pub fn build_layout(element: &Element, char_start: u32) -> LayoutQuery {
    ...
    for i in 0..child_nodes.length() {
        ...
        if let Some(el) = node.dyn_ref::<Element>() {
            ...
            let child = build_layout(el, current_char);
            current_char += child.text_len();  // <-- advance by what the child consumed
            ...
            child.text = raw;
            child.char_start = current_char;  // <-- correct start for this text node
            attach_text_node_char_fns(&mut child, text_node, current_char);
