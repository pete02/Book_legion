...
use epub::doc::{EpubDoc, NavPoint, SpineItem};
use std::{io::BufReader};
use std::fs::File;

pub fn get_first_chapter(epub: &mut EpubDoc<BufReader<File>>)->Result<u32,Box<dyn std::error::Error>>{
    ...
    while !first_page {
        println!("{:?}",toc[i]);
        match check_navpoint(epub, toc[i].clone(), &spine){
            Ok(index)=>return Ok(index as u32),
            Err(_)=>{i+=1}
        }
    }
    
    return  Err("first chapter not found".into());
    
}

fn check_navpoint(epub: &mut EpubDoc<BufReader<File>>, ch:NavPoint, spine:&Vec<SpineItem>)->Result<usize,Box<dyn std::error::Error>>{
